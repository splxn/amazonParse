...
const amazonUrl = 'http://www.amazon.com/s?field-keywords=lenovo';
...
function Product(url) {
  ...
  this.body = function () {
    let fileBody = fs.readFileSync('cached_' + self.hashedUrl + '.html');
...
function processCatalog(url) {
  ...
  function parsePage(page) {
    ...
    arrOfURLs.forEach((item) => {
      ...
      fs.open('cached_' + hash + '.html', "r+", function (err) {
        if (err) {
          request({
            method: 'GET',
            uri: item,
            ...
            fs.createWriteStream('cached_' + hash + '.html')
          )
        } else {
          return;
        }
  ...
  function writeToJSON(arrItems) {
    fs.writeFile('products.json', JSON.stringify(arrItems, null, 2), 'utf-8');
  }
